...
class DAE(TextModel):
    ...
    def __init__(self, vocab, args):
        ...
        # Modify projection layer: output embedding dimension instead of vocab_size
        self.proj = nn.Linear(args.dim_h, args.dim_emb)  # change to dim_emb
    ...
    def decode(self, z, input, hidden=None):
        ...
        input = self.drop(input) + self.z2emb(z)
        output, hidden = self.G(input, hidden)
        output = self.drop(output)
        # logits = self.proj(output.view(-1, output.size(-1)))
    # output embeddings instead of logits
        output = self.proj(output.view(-1, output.size(-1)))
        return output.view(input.size(0), input.size(1), -1), hidden
    ...
    def loss_rec(self, logits, targets):
        ...
        return loss.sum(dim=-1)  # sum over embedding dimension


    # def vector_loss(self, logits, targets):
        # diff = logits - targets
...
class AAE(DAE):
    ...
    def step(self, losses):
        # super().step(losses)
        # First execute generator's step
        self.opt.zero_grad()
        total_loss = self.loss(losses)
        total_loss.backward()
        self.opt.step()

    # Then execute discriminator's step
        self.optD.zero_grad()
